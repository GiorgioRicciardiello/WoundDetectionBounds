...
Before writing, check whether a `handoffs/` directory exists in the project root:

- If it exists: write directly to `handoffs/[YYYY-MM-DD]-[brief-description].md`
- If it does not exist: create it first (via `mkdir handoffs`), then write the file

The handoff directory is **always** in the project root, never inside `.claude/`.
